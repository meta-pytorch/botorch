...
root = f"results/raw/v2/stage3shrink2_{arm}"
allf = [
    f
...
methods = sorted(set(m for p in per["s2_ctrl"].values() for m in p))
for tol in ("budget_to_0.01", "budget_to_0.001"):
    print(f"\n  === {tol} : paired by prior vs s2_ctrl, t(8)={T8} ===")
    for m in methods:
        base = {
            p: per["s2_ctrl"][p][m][tol]
            for p in per["s2_ctrl"]
            if m in per["s2_ctrl"][p]
        }
        print(f"\n  {m}")
        print(
            f"    {'s2_ctrl':<15}{st.mean(base.values()):>8.2f}   between-prior sd {st.stdev(base.values()):>5.2f}  (control)"
        )
        for c in CELLS[1:]:
            d = [
