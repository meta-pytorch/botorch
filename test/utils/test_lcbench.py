...
class TestLCBench(BotorchTestCase):
    ...
    def test_invalid_dataset_name_takes_precedence_over_missing_deps(self) -> None:
        # A bad dataset name is a programming error the caller can act on, so it
        # must be reported even when the optional Parquet deps are absent.
        with patch.object(lcbench, "_HAS_PANDAS", False):
            with self.assertRaisesRegex(ValueError, "Invalid dataset 'not_a_dataset'"):
                load_lcbench_data("not_a_dataset")
    ...
    def test_read_parquet_cleans_up_temp_file_on_write_failure(self) -> None:
        # The download parses fine, so a temp file IS created; the failure happens
        # during the write (e.g. disk full) or the atomic replace. delete=False
        # means the temp file outlives its context manager, so without explicit
        # cleanup a stray *.tmp would be left in the user's cache directory. The
        # corrupt-download test above cannot cover this: there the failure occurs
        # before the temp file exists.
        buf = io.BytesIO()
        pd.DataFrame({"a": [1, 2, 3]}).to_parquet(buf, engine="pyarrow")
        raw = buf.getvalue()
        response = MagicMock()
        response.__enter__.return_value.read.return_value = raw
        ...
        for failing, exc in (
            ...
            ("replace", OSError("Invalid cross-device link")),
        ):
            with self.subTest(failing=failing), tempfile.TemporaryDirectory() as tmp:
                ...
                if failing == "write":
                    ...
                    # `real_ntf`/`exc` are bound as defaults so the closure
                    # captures this iteration's values (ruff B023).
                    def _boom(*args, _ntf=real_ntf, _exc=exc, **kwargs):
                        handle = _ntf(*args, **kwargs)
                        handle.write = MagicMock(side_effect=_exc)
                        return handle

                    inner = patch.object(lcbench.tempfile, "NamedTemporaryFile", _boom)
                else:
                    inner = patch.object(lcbench.os, "replace", side_effect=exc)
                with stack, inner:
                ...
                self.assertEqual(list(cache_dir.rglob("*.tmp")), [])
                self.assertFalse(
                    (cache_dir / "Fashion-MNIST" / "config.parquet.gzip").exists()
                )

    def test_check_parquet_dependencies_requires_pyarrow_too(self) -> None:
        # Every read goes through engine="pyarrow", so a pandas-without-pyarrow
        # environment must get the same actionable install hint rather than an
        # opaque engine import error from deeper in pandas.
        for pandas_ok, pyarrow_ok in ((True, False), (False, True), (False, False)):
            with self.subTest(pandas=pandas_ok, pyarrow=pyarrow_ok):
                with (
                    patch.object(lcbench, "_HAS_PANDAS", pandas_ok),
                    patch.object(lcbench, "_HAS_PYARROW", pyarrow_ok),
                ):
                    with self.assertRaisesRegex(ImportError, r"botorch\[lcbench\]"):
                        lcbench._check_parquet_dependencies_available()
        # Both present -> no raise.
        with (
            patch.object(lcbench, "_HAS_PANDAS", True),
            patch.object(lcbench, "_HAS_PYARROW", True),
        ):
            lcbench._check_parquet_dependencies_available()
