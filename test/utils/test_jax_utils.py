...
class TestJaxUtils(BotorchTestCase):
    def test_torch_to_jax(self) -> None:
        t = torch.tensor([[1.0, 2.0], [3.0, 4.0]], dtype=torch.double)
        result = torch_to_jax(t)
        self.assertEqual(result.shape, (2, 2))
        self.assertTrue(jnp.allclose(result, jnp.array([[1.0, 2.0], [3.0, 4.0]])))

    def test_torch_to_jax_with_grad(self) -> None:
        t = torch.tensor([1.0, 2.0], requires_grad=True)
        result = torch_to_jax(t)
        self.assertEqual(result.shape, (1, 2) if result.ndim == 2 else (2,))
        self.assertTrue(jnp.allclose(result, jnp.array([1.0, 2.0])))
    ...
    def test_jax_to_torch_float32(self) -> None:
    ...
    def test_roundtrip(self) -> None:
        ...
        self.assertTrue(torch.allclose(original.cpu(), recovered))
