...
class TestOrthogonalAdditiveGP(BotorchTestCase):
    def test_orthogonal_additive_gp(self) -> None:
        self._test_model()
        self._test_evaluate_first_order_on_grid()
        self._test_evaluate_first_order_ground_truth()
        self._test_evaluate_first_order_partial_observability()
        self._test_evaluate_first_order_on_grid_batch_shape()
        self._test_observation_noise_with_infer_all_components()
        self._test_per_component_multiple_inputs_error()
    ...
    def _test_model(self) -> None:
        ...
        # Verify default base kernel has per-dimension lengthscales
        base_k = model_default.covar_module.base_kernel
        self.assertIsInstance(base_k, RBFKernel)
        self.assertEqual(base_k.batch_shape, torch.Size([d]))
        self.assertEqual(base_k.lengthscale.shape, torch.Size([d, 1, 1]))
    ...
    def _test_evaluate_first_order_on_grid(self) -> None:
    ...
    def _test_evaluate_first_order_ground_truth(self) -> None:
    ...
    def _test_evaluate_first_order_partial_observability(self) -> None:
    ...
    def _test_evaluate_first_order_on_grid_batch_shape(self) -> None:
        """Test evaluate_first_order_on_grid with batch-shaped training data."""
        d = 3
        grid_size = 10
        tkwargs = {"dtype": torch.double, "device": self.device}
        ...
        for batch_shape in [(2,), (3, 2)]:
            n = 15
            train_X = torch.rand(*batch_shape, n, d, **tkwargs)
            train_Y = torch.randn(*batch_shape, n, 1, **tkwargs)
            ...
            model = OrthogonalAdditiveGP(
                train_X, train_Y, covar_module=oak, outcome_transform=None
            )
            model.eval()
            ...
            # Bias should have batch_shape (scalar per batch element)
            self.assertEqual(bias_mean.shape, torch.Size(batch_shape))
            self.assertEqual(bias_var.shape, torch.Size(batch_shape))

            # First-order should have (d, *batch_shape, grid_size)
            expected_fo_shape = torch.Size([d, *batch_shape, grid_size])
            self.assertEqual(fo_mean.shape, expected_fo_shape)
            self.assertEqual(fo_var.shape, expected_fo_shape)

            # Variances should be non-negative
            self.assertTrue((bias_var >= 0).all())
            self.assertTrue((fo_var >= 0).all())

    def _test_observation_noise_with_infer_all_components(self) -> None:
        """Test that observation_noise=True works with infer_all_components=True."""
        n, d = 10, 3
        tkwargs = {"dtype": torch.double, "device": self.device}
        train_X = torch.rand(n, d, **tkwargs)
        train_Y = torch.randn(n, 1, **tkwargs)
        ...
        test_X = torch.rand(5, d, **tkwargs)
        expected = model.num_components

        # Without noise
        post_no_noise = model.posterior(test_X, infer_all_components=True)
        self.assertEqual(post_no_noise.mean.shape, torch.Size([expected, 5, 1]))

        # With noise
        post_noise = model.posterior(
            test_X, infer_all_components=True, observation_noise=True
        )
        self.assertEqual(post_noise.mean.shape, torch.Size([expected, 5, 1]))

        # Variance with noise should be >= variance without noise
        self.assertTrue(
            (post_noise.variance >= post_no_noise.variance - 1e-6).all(),
            "Variance with noise should be >= variance without noise",
        )

    def _test_per_component_multiple_inputs_error(self) -> None:
        """Test ValueError when multiple train/test inputs are passed."""
        n, d = 10, 3
        tkwargs = {"dtype": torch.double, "device": self.device}
        train_X = torch.rand(n, d, **tkwargs)
        train_Y = torch.randn(n, 1, **tkwargs)

        model = OrthogonalAdditiveGP(train_X, train_Y, outcome_transform=None)
        model.eval()

        # _get_test_prior_mean_and_covariances_per_component expects single inputs
        with self.assertRaises(ValueError):
            model._get_test_prior_mean_and_covariances_per_component(
                train_inputs=[train_X, train_X],
                test_inputs=[train_X],
            )
