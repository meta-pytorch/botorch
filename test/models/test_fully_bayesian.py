...
class TestPyroModelWarp(BotorchTestCase):
    ...
    def test_warp_basic(self) -> None:
        ...
        c0 = torch.ones(d, dtype=torch.double, device=self.device) * 2.0
        c1 = torch.ones(d, dtype=torch.double, device=self.device) * 3.0
    ...
    def test_warp_identity_concentrations(self) -> None:
        """Test that c0=c1=1 approximates the identity (modulo eps clamping)."""
        eps = 1e-7
        pyro_model = CustomPyroModel(eps=eps)
        n, d = 10, 2
        # Use values away from 0 and 1 to avoid eps effects
        X = torch.rand(n, d, dtype=torch.double, device=self.device) * 0.8 + 0.1
        c0 = torch.ones(d, dtype=torch.double, device=self.device)
        c1 = torch.ones(d, dtype=torch.double, device=self.device)

        warped = pyro_model.warp(X, c0=c0, c1=c1)
        # Kumaraswamy CDF with c0=c1=1 is the identity: 1 - (1 - x)^1 = x
        # (up to the eps clamping in kumaraswamy_warp)
        self.assertAllClose(warped, X, atol=3 * eps)

    def test_warp_with_indices(self) -> None:
        """Test that warp only transforms the specified indices."""
        n, d = 5, 4
        indices = [0, 2]
        pyro_model = CustomPyroModel(indices_to_warp=indices)
        X = torch.rand(n, d, dtype=torch.double, device=self.device)
        c0 = torch.tensor([2.0, 3.0], dtype=torch.double, device=self.device)
        c1 = torch.tensor([3.0, 2.0], dtype=torch.double, device=self.device)

        warped = pyro_model.warp(X, c0=c0, c1=c1)
        self.assertEqual(warped.shape, X.shape)
        # Non-warped dimensions should be unchanged
        self.assertAllClose(warped[:, 1], X[:, 1])
        self.assertAllClose(warped[:, 3], X[:, 3])
        # Warped dimensions should differ from original
        self.assertFalse(torch.allclose(warped[:, 0], X[:, 0]))
        self.assertFalse(torch.allclose(warped[:, 2], X[:, 2]))

    def test_warp_output_range(self) -> None:
        """Test that warp output is in [0, 1] for various concentrations."""
        pyro_model = CustomPyroModel()
        n, d = 20, 3
        X = torch.rand(n, d, dtype=torch.double, device=self.device)
        for c0_val, c1_val in [(0.5, 0.5), (2.0, 3.0), (10.0, 0.1)]:
            c0 = torch.full((d,), c0_val, dtype=torch.double, device=self.device)
            c1 = torch.full((d,), c1_val, dtype=torch.double, device=self.device)
            warped = pyro_model.warp(X, c0=c0, c1=c1)
            self.assertTrue((warped >= 0).all())
            self.assertTrue((warped <= 1).all())

    def test_warp_batch_shape(self) -> None:
        """Test that warp handles batched inputs."""
        pyro_model = CustomPyroModel()
        b, n, d = 3, 5, 2
        X = torch.rand(b, n, d, dtype=torch.double, device=self.device)
        c0 = torch.ones(d, dtype=torch.double, device=self.device) * 2.0
        c1 = torch.ones(d, dtype=torch.double, device=self.device) * 3.0
    ...
    def test_warp_boundary_inputs(self) -> None:
        """Test that warp handles inputs at 0 and 1 (clamped by eps)."""
        eps = 1e-7
        pyro_model = CustomPyroModel(eps=eps)
        d = 2
        X = torch.tensor(
        ...
        c0 = torch.ones(d, dtype=torch.double, device=self.device) * 2.0
        c1 = torch.ones(d, dtype=torch.double, device=self.device) * 2.0
    ...
    def test_warp_differentiable(self) -> None:
        """Test that warp is differentiable w.r.t. inputs."""
        pyro_model = CustomPyroModel()
        n, d = 5, 3
        X = torch.rand(n, d, dtype=torch.double, device=self.device, requires_grad=True)
        c0 = torch.full((d,), 2.0, dtype=torch.double, device=self.device)
        c1 = torch.full((d,), 3.0, dtype=torch.double, device=self.device)
