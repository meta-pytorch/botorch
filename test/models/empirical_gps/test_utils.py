...
class TestUtils(BotorchTestCase):
    def test_compute_svd_basis_vectors(self) -> None:
        # Test that SVD basis vectors produce the same Gram matrix as original
        torch.manual_seed(42)  # Fix seed for reproducibility

        for dtype in [torch.float32, torch.float64]:
            tol = 1e-3 if dtype == torch.float32 else 1e-10

            # Test case 1: tall matrix (m >> n) - typical use case
            m, n = 1000, 50
            A = torch.randn(m, n, dtype=dtype, device=self.device)

            B = compute_svd_basis_vectors(A)
    ...
    def test_covariance_computation_helpers(self) -> None:
        ...
        num_curves = 5
        num_progression = 10
        Y_2d = torch.randn(num_curves, num_progression, dtype=torch.float64)
        ...
        self.assertAllClose(
            ...
            torch.zeros(num_progression, dtype=torch.float64),
            atol=1e-10,
        )
        ...
        Y_3d = torch.randn(num_curves, num_progression, m, dtype=torch.float64)
        ...
        self.assertAllClose(
            ...
            torch.zeros(num_progression, m, dtype=torch.float64),
            atol=1e-10,
        )

        # Test compute_sample_covariance - full covariance
        n1, n2 = 4, 6
        U1 = torch.randn(num_curves, n1, dtype=torch.float64)
        U2 = torch.randn(num_curves, n2, dtype=torch.float64)
        ...
        Y_for_interp = torch.randn(1, num_curves, num_progression, dtype=torch.float64)
        f = LinearInterpolation1D(
            torch.linspace(0, 1, num_progression),
            Y_for_interp,
        )
        ...
        curve_weights = torch.rand(num_curves, dtype=torch.float64)
        ...
        dummy = DummyModule()
        instantiate_ard(
            dummy, num_curves=num_curves, curve_weights=None, dtype=Y_2d.dtype
        )
        ...
        self.assertAllClose(
            dummy.curve_weights, torch.ones(num_curves, dtype=torch.float64)
        )

        # Test instantiate_ard with 3D tensor
        dummy_3d = DummyModule()
        instantiate_ard(
            dummy_3d, num_curves=num_curves, curve_weights=None, dtype=Y_3d.dtype
        )
        ...
        custom_weights = torch.nn.Parameter(torch.rand(num_curves, dtype=torch.float64))
        instantiate_ard(
