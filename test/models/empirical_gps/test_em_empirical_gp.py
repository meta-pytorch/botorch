...
class TestEMCovarianceShrinkage(BotorchTestCase):
    """Tests for base-kernel covariance shrinkage in the EM model.

    Covers both the M-step shrinkage (during pre-training) and the post-EM
    conditioning shrinkage (with an optionally MLL-fit intensity).
    """

    def _prior(self, tkwargs, K=6, n=8, shrinkage=0.0):
        torch.manual_seed(0)
        X = torch.linspace(0.0, 1.0, n, **tkwargs).unsqueeze(-1)
        datasets = [
            ExperimentDataset(
                X=X, Y=(torch.sin(3.0 * X) + 0.1 * torch.randn(n, 1, **tkwargs))
            )
            for _ in range(K)
        ]
        return (
            pretrain_em_prior(
                datasets=datasets,
                mean_module=ConstantMean().to(**tkwargs),
                covar_module=ScaleKernel(MaternKernel(nu=2.5)).to(**tkwargs),
                num_em_iterations=5,
                inducing_points=X,
    ...
    def test_mstep_shrinkage(self) -> None:
        tkwargs = {"dtype": torch.double, "device": self.device}
        p0, _ = self._prior(tkwargs, shrinkage=0.0)
        p5, _ = self._prior(tkwargs, shrinkage=0.5)
        # M-step shrinkage changes the learned covariance ...
        self.assertFalse(torch.allclose(p0.Sigma_inducing, p5.Sigma_inducing))
        # ... while keeping it symmetric PSD.
        for p in (p0, p5):
            self.assertTrue(torch.allclose(p.Sigma_inducing, p.Sigma_inducing.mT))
            evals = torch.linalg.eigvalsh(p.Sigma_inducing)
            self.assertGreater(evals.min().item(), -1e-6)

    def test_conditioning_shrinkage(self) -> None:
        from gpytorch.kernels import MaternKernel, ScaleKernel

        tkwargs = {"dtype": torch.double, "device": self.device}
        prior, X = self._prior(tkwargs)
        train_X = X[:4]
        train_Y = torch.sin(3.0 * train_X).squeeze(-1)
        # base_covar_module=None (default) recovers the pure EM-EGP.
        m0 = EMEmpiricalGaussianProcess.from_pretrained(
            em_prior=prior, train_X=train_X, train_Y=train_Y
        )
        self.assertIsNone(m0._additive_base)
        # An additive base kernel changes the predictive covariance.
        base = ScaleKernel(MaternKernel(nu=2.5)).to(**tkwargs)
        ma = EMEmpiricalGaussianProcess.from_pretrained(
            em_prior=prior, train_X=train_X, train_Y=train_Y, base_covar_module=base
        )
        self.assertIs(ma._additive_base, base)
        m0.eval()
        ma.eval()
        with torch.no_grad():
            v0 = m0.posterior(X).variance
            va = ma.posterior(X).variance
        self.assertFalse(torch.allclose(v0, va))

    def test_fit_base_kernel(self) -> None:
        from gpytorch.kernels import MaternKernel, ScaleKernel
        ...
        train_Y = torch.sin(3.0 * train_X).squeeze(-1)
        base = ScaleKernel(MaternKernel(nu=2.5)).to(**tkwargs)
        # Freeze the base lengthscales; leave only its outputscale trainable (the
        # "fit the magnitude, not the whole kernel" mode). Which params are fit is
        # controlled purely by requires_grad on the caller-supplied base kernel.
        ...
        loss = -mll(None, None)
        loss.backward()
        self.assertIsNotNone(base.raw_outputscale.grad)
        self.assertTrue(torch.isfinite(base.raw_outputscale.grad).all())
        self.assertIsNone(base.base_kernel.raw_lengthscale.grad)

    def test_init_covariance_shrinkage(self) -> None:
        # M-step shrinkage must be reachable from the constructor (not only from
        # pretrain_em_prior), threading through to the M-step blend.
        tkwargs = {"dtype": torch.double, "device": self.device}
        torch.manual_seed(0)
        n, K = 8, 6
        X = torch.linspace(0.0, 1.0, n, **tkwargs).unsqueeze(-1)
        ...
        def _make(shrinkage: float) -> EMEmpiricalGaussianProcess:
            torch.manual_seed(0)
            return EMEmpiricalGaussianProcess(
                train_X=train_X,
                train_Y=train_Y,
        ...
        m0 = _make(0.0)
        m5 = _make(0.5)
        # Constructor-threaded M-step shrinkage changes the learned covariance ...
        self.assertFalse(torch.allclose(m0._Sigma_inducing, m5._Sigma_inducing))
        # ... while keeping it symmetric PSD.
        for m in (m0, m5):
            self.assertTrue(torch.allclose(m._Sigma_inducing, m._Sigma_inducing.mT))
            evals = torch.linalg.eigvalsh(m._Sigma_inducing)
            self.assertGreater(evals.min().item(), -1e-6)

    def test_run_em_reapplies_shrinkage(self) -> None:
        # The training-mode EM re-run (_run_em) must re-apply M-step shrinkage,
        # not silently drop it. Regression guard: covariance_shrinkage used to be
        # threaded only into the initial pretrain, so a from-scratch model re-ran
        # EM without shrinkage on every optimizer step.
        tkwargs = {"dtype": torch.double, "device": self.device}
        torch.manual_seed(0)
        n, K = 8, 6
        X = torch.linspace(0.0, 1.0, n, **tkwargs).unsqueeze(-1)
        ...
        def _sigma_from_refit(shrinkage: float) -> torch.Tensor:
            torch.manual_seed(0)
            model = EMEmpiricalGaussianProcess(
                train_X=train_X,
                train_Y=train_Y,
            ...
            # From-scratch model: forward() re-runs EM in training mode.
            self.assertFalse(model._using_pretrained_prior)
            model.train()
            _, sigma = model._run_em()
            return sigma

        s0 = _sigma_from_refit(0.0)
        s5 = _sigma_from_refit(0.5)
        # The re-fit path now reflects shrinkage (would be identical if dropped) ...
        self.assertFalse(torch.allclose(s0, s5))
        # ... and stays symmetric PSD.
        self.assertTrue(torch.allclose(s5, s5.mT))
        self.assertGreater(torch.linalg.eigvalsh(s5).min().item(), -1e-6)

    def test_batched_posterior_and_logei(self) -> None:
        # The interpolated posterior must support t-batched ``(b, q, d)`` inputs so
        # analytic acquisitions (e.g. LogExpectedImprovement) can evaluate the EM
        # model over a candidate set. Regression guard for the non-batch-aware
        # ``_interpolate_prior`` that broke on ``(b, 1, d)`` shapes.
        ...
        model.eval()
        with torch.no_grad():
            flat = model.posterior(X)  # joint over n points (non-batched path)
            batched = model.posterior(X.unsqueeze(-2))  # (n, 1, d) t-batches
        n = X.shape[0]
        # BoTorch posteriors carry a trailing output dim: (t-batch, q, m).
        self.assertEqual(batched.mean.shape, torch.Size([n, 1, 1]))
        self.assertEqual(batched.variance.shape, torch.Size([n, 1, 1]))
        self.assertTrue(torch.isfinite(batched.mean).all())
        self.assertTrue(torch.isfinite(batched.variance).all())
        # Batched marginals match the joint posterior's diagonal marginals.
        self.assertAllClose(batched.mean.reshape(-1), flat.mean.reshape(-1), atol=1e-6)
        self.assertAllClose(
            batched.variance.reshape(-1), flat.variance.reshape(-1), atol=1e-6
        )
        # LogExpectedImprovement now evaluates on the EM model without error.
        acqf = LogExpectedImprovement(model, best_f=float(train_Y.max()), maximize=True)
        vals = acqf(X.unsqueeze(-2))
        self.assertEqual(vals.shape, torch.Size([n]))
        self.assertTrue(torch.isfinite(vals).all())

    def test_base_covar_module_adds_full_kernel_offgrid(self) -> None:
        # Option A: the additive base kernel contributes its FULL K_base(X, X) at
        # query points, not the Nystrom projection W K_base(Z, Z) W^T. This keeps
        # the base variance intact away from the (possibly sparse, higher-dim)
        # inducing set. Regression guard for the previously Nystrom-limited base.
        ...
        # Short lengthscale so the Nystrom projection would badly under-count.
        base.base_kernel.lengthscale = 0.05
        base.outputscale = 2.0
        model_no_base = EMEmpiricalGaussianProcess.from_pretrained(
            em_prior=prior, train_X=train_X, train_Y=train_Y
        )
        model_base = EMEmpiricalGaussianProcess.from_pretrained(
            em_prior=prior, train_X=train_X, train_Y=train_Y, base_covar_module=base
        )
        # Query strictly between inducing points (off-grid, where W != I).
        X_off = (X[:-1] + X[1:]) / 2.0
        with torch.no_grad():
            _, Sigma_no_base = model_no_base._interpolate_prior_to_X(X_off)
            _, Sigma_base = model_base._interpolate_prior_to_X(X_off)
            k_base = base(X_off, X_off).to_dense()
        # The base kernel adds exactly its full K_base(X_off, X_off) -- the added
        # off-grid prior variance is the full outputscale, which the Nystrom
        # projection W K_base(Z, Z) W^T would strictly under-count.
        self.assertAllClose(Sigma_base - Sigma_no_base, k_base, atol=1e-6)

    def test_base_covar_module_adds_full_kernel_ongrid(self) -> None:
        # The additive base also augments the on-grid fast path
        # (_get_prior_at_indices -> _effective_Sigma_inducing), not only the
        # off-grid shift-interpolation path.
        from gpytorch.kernels import RBFKernel, ScaleKernel
        ...
        # Without a base the effective covariance is the raw EM covariance itself.
        self.assertIs(
            model_no_base._effective_Sigma_inducing(), model_no_base._Sigma_inducing
        )

        indices = torch.tensor([0, 2, 3], device=self.device)
        with torch.no_grad():
            _, Sigma_no_base = model_no_base._get_prior_at_indices(indices)
            _, Sigma_base = model_base._get_prior_at_indices(indices)
