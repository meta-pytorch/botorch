...
def _check_parquet_dependencies_available() -> None:
    # Both are required: every read goes through ``pd.read_parquet(...,
    # engine="pyarrow")``. Checking only pandas meant a pandas-without-pyarrow
    # environment passed this guard and failed later with an opaque engine
    # import error instead of the actionable install hint below.
    if not (_HAS_PANDAS and _HAS_PYARROW):
        raise ImportError(
...
def _read_lcbench_parquet(
    ...
    # delete=False means the temp file outlives the context manager, so any
    # failure between creation and the atomic replace would leave a stray
    # ``*.tmp`` in the user's cache directory. Clean up on every error path --
    # but only once the handle is closed, because Windows refuses to unlink a
...
def load_lcbench_data(
    ...
    _check_parquet_dependencies_available()
