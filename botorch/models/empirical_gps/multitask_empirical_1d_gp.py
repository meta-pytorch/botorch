...
def _extract_progression_and_task(
    ...
    if task_feature_values.is_floating_point():
        ...
        if not torch.allclose(task_feature_values, rounded, atol=1e-6, rtol=0.0):
            # NaN compares False against every threshold, so it has to be picked
            # up explicitly; otherwise all-NaN labels report an empty `bad` list.
            offending = ((task_feature_values - rounded).abs() > 1e-6) | (
                task_feature_values.isnan()
            )
            bad = task_feature_values[offending].unique().tolist()
            raise ValueError(
...
class MultiTaskEmpiricalOneDimensionalGP(ExactGP, MultiTaskGPyTorchModel):
    ...
    Note:
        This model is designed for cross-task transfer of a *specific
        configuration*: the setting where a configuration is observed on donor
        tasks but not (or barely) on the target, and the historical cross-task
        covariance carries the information across. A single-task model cannot
        address that setting at all. See the empirical GP tutorial for a worked
        zero/few-shot example against parametric multi-task baselines.

        Where the target task is already well observed, the advantage over a
        single-task :class:`EmpiricalOneDimensionalGP` is much less clear-cut and
        depends on the protocol. One caution that does generalize: without a
        ``base_covar_module``, the rank-limited empirical covariance can leave the
        model overconfident as observations accumulate. Supplying a fitted
        additive base kernel addresses this, and is what the tutorial uses.
    """
