...
class TestPriorFittedNetwork(BotorchTestCase):
    ...
    def test_shapes(self):
        ...
        X, train_X, train_Y, orig_X_shape, style, y_style = pfn._prepare_data(X)
        ...
        self.assertIsNone(y_style)
        pfn.style = torch.rand(4, **tkwargs)
        pfn.y_style = torch.rand(4, **tkwargs)
        X, train_X, train_Y, orig_X_shape, style, y_style = pfn._prepare_data(X)
        self.assertEqual(style.shape, torch.Size([1, 4]))
        self.assertEqual(y_style.shape, torch.Size([1, 4]))
    ...
    def test_estimate_correlations(self):
        ...
        with patch(
            ...
            R = self.pfn.estimate_correlations(
                ...
                style=torch.zeros(2, 4),
                y_style=torch.zeros(2, 4),
                marginals=marginals,
            )
